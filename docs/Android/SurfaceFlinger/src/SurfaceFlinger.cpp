...
void SurfaceFlinger::bootFinished() {
    ...
    /// AW CODE:[DroidBoost V2.0.1]add bootevent for bootanim
...
void SurfaceFlinger::init() FTL_FAKE_GUARD(kMainThreadContext) {
    ...
    /// AW CODE:[Bug#182815]fix hotplug-in without effect when init
    bool scheduleConfigure;
    {
        std::lock_guard<std::mutex> lock(mHotplugMutex);
        scheduleConfigure = !mPendingHotplugEvents.empty();
    }
    if (scheduleConfigure) {
...
ui::Rotation SurfaceFlinger::getPhysicalDisplayOrientation(DisplayId displayId,
    ...
    ui::Rotation mPrimaryOrientation = ui::ROTATION_0;
    ...
    if (!mIgnoreHwcPhysicalDisplayOrientation &&
        ...
        switch (getHwComposer().getPhysicalDisplayOrientation(*id)) {
            case Hwc2::AidlTransform::ROT_90:
                mPrimaryOrientation = ui::ROTATION_90;
                break;
            case Hwc2::AidlTransform::ROT_180:
                mPrimaryOrientation = ui::ROTATION_180;
                break;
            case Hwc2::AidlTransform::ROT_270:
                mPrimaryOrientation = ui::ROTATION_270;
                break;
            default:
                mPrimaryOrientation = ui::ROTATION_0;
                break;
        }
    } else if (isPrimary) {
    ...
    if (isPrimary) {
        switch (user_rotation) {
                case 90:
                    mPrimaryOrientation = mPrimaryOrientation + ui::ROTATION_270;
                    break;
                case 180:
                    mPrimaryOrientation = mPrimaryOrientation + ui::ROTATION_180;
                    break;
                case 270:
                    mPrimaryOrientation = mPrimaryOrientation + ui::ROTATION_90;
                    break;
                default:
                    break;
         }
    }
     return mPrimaryOrientation;
}
...
sp<DisplayDevice> SurfaceFlinger::setupNewDisplayDeviceInternal(
    ...
    ALOGE("Display Orientation: %s", toCString(creationArgs.physicalOrientation));
    ...
    flushState.transaction->traverseStatesWithBuffersWhileTrue([&](const layer_state_t& s,
        ...
        if (s.bufferData->hasBarrier) {
            ...
            if (layer->getDrawingState().barrierFrameNumber < s.bufferData->barrierFrameNumber) {
                ...
                /// AW CODE: [fix] fix sf memory leak
                const int MAX_BARRIER_TIME = 60 * 3;
                /* sf will never show next frame if app never update barrierFrameNumber.
                 * screen will blocked, buffers will maped in sf and never release
                 * release dead lock here*/
                if (layer->getDrawingState().barrierShoted < MAX_BARRIER_TIME) {
                    layer->getDrawingState().barrierShoted++;
                } else {
                    ALOGD("force %s to update, barrierFrameNumber:%" PRId64 " < %" PRId64,
                                  layer->getDebugName(),
                                  layer->getDrawingState().barrierFrameNumber,
                                  s.bufferData->barrierFrameNumber);
                    layer->getDrawingState().barrierFrameNumber = s.bufferData->barrierFrameNumber + 1;
                }
                /// AW CODE
        ...
        if (!fenceSignaled) {
            ...
            if (allowLatchUnsignaled) {
                ...
                ///AW CODE: Don't indicate gpu hang when glbenchmark,
                  //since gpu may take more than 4s to finish rendering
                /*if (listener &&
                ...
                if (listener &&
                    (flushState.queueProcessTime - transaction.postTime) >
                            std::chrono::nanoseconds(4s).count() &&
                           (!layer->getName().find("glbenchmark"))) {
               ///AW: add end
                    mTransactionHandler
...
status_t SurfaceFlinger::doDump(int fd, const DumpArgs& args, bool asProto) {
    ...
    if (numArgs && ((args[0] == String16("--file")) ||
        (args[0] == String16("--allocated-buffers")))) {
    ...
    if ((uid != AID_SHELL) &&
        ...
        static const std::unordered_map<std::string, Dumper> dumpers = {
                ...
                {"--layer"s, argsDumper(&SurfaceFlinger::dumpHwcLayers)},
        };
...
status_t SurfaceFlinger::doDumpContinuous(int fd, const DumpArgs& args) {
    // command: dumpsys SurfaceFlinger --file --no-limit
    size_t numArgs = args.size();
    status_t err = NO_ERROR;

    if (args[0] == String16("--allocated-buffers")) {
        std::string dumpsys;
...
void SurfaceFlinger::dumpDrawCycle(bool prePrepare) {
    ...
    std::string compositionLayers;
    {
        ...
        if (mFileDump.fullDump) {
            ...
            dumpHwcLayersMinidumpLocked(dumpsys);
        }
    }
...
void SurfaceFlinger::dumpHwcLayersMinidumpLocked(std::string& result) const {
    /* dump Display state */
    StringAppendF(&result, "Display (%zu entries)\n", mDisplays.size());
    utils::Dumper dumper{result};
    for (const auto& [token, display] : mDisplays) {
        const auto displayId = display->getId();
        utils::Dumper::Section section(dumper,
    ...
    bool hwcDisabled = mDebugDisableHWC;
    StringAppendF(&result, "  h/w composer %s\n", hwcDisabled ? "disabled" : "enabled");
    getHwComposer().dump(result);
}

/// AW CODE: vendor command to dump hwc-hal debug info
void SurfaceFlinger::dumpHwcLayers(const DumpArgs& args, std::string& result) const {
    /*
     * HWC layer minidump
     */
    for (const auto& [token, display] : mDisplays) {
...
#pragma clang diagnostic error "-Wswitch-enum"
    ...
    if (code >= 1000 && code <= 1100/*AW_CODE:appstart:experience:extend code 1100;jscese;210818;1042*/) {
...
status_t SurfaceFlinger::onTransact(uint32_t code, const Parcel& data, Parcel* reply,
    ...
    if (err == UNKNOWN_TRANSACTION || err == PERMISSION_DENIED) {
        ...
        switch (code) {
           ...
           /*AW_code:add skip animation-frame for app-start performance;jscese;200915
            * add displayid for new interface 230629*/
            case 1100: {
                Mutex::Autolock lock(mStateLock);
                float scale = data.readFloat();
                ALOGD("setRefreshSkipScale %f", scale );
                mScheduler->setRefreshSkipScale(mActiveDisplayId,scale);
                return NO_ERROR;
            }
